...
| `offset` 해석 | 페이지 번호 (`PageRequest.of(offset, count)`) | 이름은 행 오프셋을 뜻하지만 SPEC이 못박지 않았다. E2E가 쓰는 `offset=0`에서는 두 해석의 결과가 같다. 행 오프셋 의미로 바꿔야 하면 API 변경 없이 Service만 고치면 된다 |
