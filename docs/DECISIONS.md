...
### 재주문 시 단가를 무엇으로 남길 것인가

`(customer_id, product_id)` 복합 UNIQUE 때문에 고객당 상품은 **1행**이고, 따라서 단가도 하나뿐이다.
가격이 바뀐 뒤 재주문하면 어느 값을 남길지 정해야 한다. **가중평균**을 쓴다.

```
orderedPrice = (기존단가 × 기존수량 + 현재가 × 추가수량) / (기존수량 + 추가수량)
```

다른 두 방식은 모두 **지금 고치는 결함과 같은 형태**로 실패한다:

| 방식 | 실패 사례 |
|---|---|
| 최초가 유지 | 15,000에 2개 → 10,000으로 하락 → 1개 추가. 결제 40,000인데 전량 취소 시 45,000 환불 |
| 현재가 덮어쓰기 | 15,000에 2개 → 50,000으로 상승 → 1개 추가. 결제 80,000인데 전량 취소 시 150,000 환불 |

가중평균은 **전량 취소 시 환불 총액이 결제 총액과 일치하는 유일한 선택**이다.
실측: 15,000×2 + 10,000×1 = 40,000 결제 → 단가 13,333.33… 저장 → 3개 전량 취소 → 원금 정확히 복귀.

> **한계** — 부분 취소는 평균가로 환불되므로 "어느 시점에 산 물건을 취소하는가"를 구분하지 못한다.
> 정확히 하려면 주문 건별 행(`Order` + `OrderLine`)이 필요하다. 현재 구조가 "주문 이력"이 아니라
> "현재 보유 수량"이라는 한계와 같은 뿌리다 (13절).
