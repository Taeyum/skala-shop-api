...
`OrderItemRepository.findByCustomer`에 페치 조인을 건다. **매핑의 LAZY는 그대로 둔다** —
`Product`를 EAGER로 바꾸면 상품이 필요 없는 조회에서도 조인이 붙어 전역적으로 나빠진다.
페치 전략은 매핑이 아니라 **쿼리마다** 정한다.

```
✓ 반환 행 수 20 건 (= 주문한 상품 수)
GET /api/customers/{id}   쿼리 2 개   [상품 20 종]
✓ 대조군 반환 행 수 10 건
GET /api/products/list    쿼리 2 개    (대조군)
```
...
총 2 개
    1회  select c1_0.id, c1_0.customer_id, ... from customers c1_0 where c1_0.customer_id=?
    1회  select oi1_0.id, oi1_0.customer_id, oi1_0.ordered_amount, oi1_0.product_id,
                p1_0.id, p1_0.product_name, p1_0.product_price, oi1_0.quantity
          from order_items oi1_0
          join products p1_0 on p1_0.id=oi1_0.product_id
          where oi1_0.customer_id=?
```

`products` 개별 조회 20회가 **조인 하나로 접혔다.**

> `join`이 `left join`이 아닌 이유 — `@JoinColumn(name = "product_id", nullable = false)`이라
> Hibernate가 연관을 필수로 보고 내부 조인을 쓴다. **연관이 optional이었다면 내부 조인은
> product가 없는 OrderItem을 조용히 누락시킨다.** 여기서는 FK가 NOT NULL이라 안전하지만,
> 페치 조인을 걸 때 행 수가 줄지 않는지 확인해야 하는 이유가 이것이다 —
> 그래서 이 측정은 쿼리 수보다 **반환 행 수를 먼저** 단언한다.
...
**개선 전은 N에 비례(N+2), 개선 후는 N과 무관하게 상수 2.**
행 수는 양쪽 모두 N과 일치하므로 쿼리가 줄어든 것이 "일을 덜 한" 결과가 아니다.
대조군이 움직이지 않아 변화의 출처가 `findByCustomer`의 페치 전략임이 확정된다.

---

## 측정 중 발견한 것 — 대조군이 흔들렸다

개선 후 첫 측정에서 대조군이 **2가 아니라 1**로 나왔다. 변하지 않아야 할 값이 변했으므로
결과를 채택하지 않고 원인을 먼저 찾았다.

원인은 코드가 아니라 **측정 픽스처였다.** 측정 스크립트는 실행할 때마다 상품 N종을 새로
만드는데, 이것이 **같은 DB에 누적**된다. 그래서 실행마다 대조군이 서로 다른 데이터 위에서
측정되고 있었다.

그 차이가 쿼리 수를 바꾼 이유는 Spring Data의 최적화다 — `offset=0`이고 결과가 한 페이지에
모두 들어가면 **count 쿼리를 생략한다.**
...
첫 측정 당시 상품은 시드 3종 + 측정용 5종 = 8종이라 `count=10` 한 페이지에 다 들어갔다.
**대조군을 "총 상품 수 > count" 조건에서 재측정**한 뒤 세 실행 모두 2로 안정됐다.

같은 오염이 E2E도 깨뜨렸다 — `SPEC.md` 5절의 "상품 3개 확인"이 **93개**로 실패했다.
코드 회귀가 아니라 측정 잔여물이었고, `ddl-auto: create`이므로 앱 재기동으로 복구된다(48/48).

**배운 것** — 대조군은 "값이 같기를 기대하는 경로"가 아니라 **"측정 조건이 같은 경로"** 여야 한다.
측정이 공유 상태를 바꾸면 대조군도 함께 오염된다. 대조군이 흔들린 것 자체가 측정 설계 결함을
알려준 신호였고, 그 신호가 없었다면 오염된 조건에서 나온 "2 vs 1"을 개선 효과로 읽었을 것이다.
