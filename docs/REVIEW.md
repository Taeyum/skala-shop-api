...
> **[정정 — 2026-08-08 보고서 검증]** 처음에는 2번(측정 픽스처가 DB를 오염)이 **어느 부류에도 없었다.**
> 건수를 적지 않아 3 + 9 = 12로 13에 못 미치는 것이 드러나지 않았고, 보고서가 건수를 붙이면서 잡혔다.
> 2번은 **대조군이 2 → 1이라는 수치를 냈고 그 수치가 틀렸으므로** 두 번째 부류다.
> **목록을 만들 때 총계와 대조하지 않은 것**이 원인이다 — 이 표가 말하는 결함과 같은 종류다.
...
## Phase 6 — 자체 점검 및 마감 (2026-08-08)

### 계약 준수
- [x] SPEC.md의 URI·JSON 필드 변경 없음. **Phase 6 전체에서 응용 코드 변경은 두 건뿐**이다 —
      동시 첫 주문 409 매핑(24절), `@Cacheable` 제거(25절). 둘 다 계약을 바꾸지 않는다
- [x] `docker compose up` 한 줄 실행 유지. HA는 독립 파일
- [x] E2E 48/48 · 테스트 160건 · 커버리지 게이트 통과

### 절대 규칙
- [x] ArchUnit 12건 그대로 통과. 캐시 제거로 `global/config`가 하나 줄었을 뿐 경계는 불변
- [x] 금액 `BigDecimal` · `open-in-view: false` 유지
- [x] 설정값에 근거 — **풀·스레드 기본값을 바꾸지 않은 것 자체가 이 규칙의 적용이다**(26절)

### 이 Phase가 실제로 산출한 것

| 산출물 | 내용 |
|---|---|
| **결함 2건** | 동시 첫 주문 500 → 409 · `@Cacheable` 제거 |
| **판단 3건** | 튜닝값 미확정 · 캐시 제거 · graceful 관측 조건 부재 |
| **반증 1건** | 26절 "409는 DB 동시성 폭의 특이 지표" 철회 |
| **도구 9건** | k6 하네스·스윕·`retro-check`·`pyguard`·`shutdown` 계열 |
| **도구 결함 9건** | 측정 장치 오류 표 5~13번 |

**측정으로 얻은 것보다 측정 도구를 고치며 얻은 것이 많은 Phase였다.**

### 미해결 항목 — 각각 "왜 지금 하지 않는가"와 함께

| # | 항목 | 왜 지금 하지 않는가 |
|---|---|---|
| 1 | **`offset` 해석** (페이지 번호 vs 건너뛸 개수) | **완성 프로젝트가 제공되지 않아 정답을 확인할 수단이 없다.** 강의 자료는 "Pageable 객체 생성"까지만 명시하고 `PageRequest.of()` 인자를 제시하지 않는다. **현행은 페이지 번호**(`PageRequest.of(offset, count)`, Spring Data 관례)이며 근거는 `DECISIONS.md` 8절 |
| 2 | **`GET /api/customers/list` 익명 공개** | 올바른 해법은 **역할 기반 접근 제어**이고 이 프로젝트에 역할 개념이 없다. "로그인만 요구"는 가입이 자유로워 실질적 방어가 되지 못한다. 계약(SPEC)도 인증 불필요로 고정돼 있다 |
| 3 | **로그인 타이밍 부채널** | 방어의 전제가 **요청 속도 제한**이다. 그것 없이 응답 시간만 균일화하면 공격 비용만 조금 올릴 뿐이다. 속도 제한은 이 프로젝트 범위 밖(게이트웨이 계층) |
| 4 | **409 상승 기전 미상** | 26절의 인과 설명이 **반증됐고 대체 설명도 기각됐다**. 가르려면 동시성 폭과 지속시간을 각각 고정한 측정이 필요한데, 그 결론이 이 프로젝트의 판단(기본값 유지)을 바꾸지 않는다 |
| 5 | **미실행 경로 지도** (`branch-audit.py`) | **덮어야 할 빚이 아니라 다음에 볼 곳의 목록이다.** 커버리지 게이트는 회귀 방지용 바닥이고, 100%는 목표가 아니다(16절). 남은 미실행 분기는 대부분 방어적 예외 경로다 |
| 6 | **오프라인·폐쇄망 빌드** | **근거 없는 대비를 하지 않는다**(22절). 채점 환경이 폐쇄망이라는 근거가 없고, 대응 절차는 이미 문서화했다 |

**여섯 항목의 공통점** — 전부 "할 수 있는데 안 한 것"이 아니라
**"지금 하면 근거 없는 결정이 되는 것"** 이다. 1·4는 판단 근거가 없고,
2·3은 전제가 되는 다른 기능이 없으며, 5·6은 하지 않는 것이 판단이다.

### 남은 한계 — 구조적인 것

| 한계 | 상태 |
|---|---|
| **DB 단일 지점** | 복제·페일오버 없음. HA로도 해결되지 않는다(27절) |
| **LB 단일 지점** | nginx가 죽으면 앱이 살아 있어도 접근 불가 |
| **`ddl-auto: create`** | 다중 인스턴스에서 롤링 재시작 불가. 무중단 배포의 전제가 빠져 있다 |
| **단일 머신 측정** | 수평 확장의 처리량 이득을 관측할 수 없다. 튜닝값이 환경 종속임이 실증됐다 |
| **기준선 재현 불가** | `phase0-baseline`이 현재 머신에서 빌드되지 않는다(29절) |

### Phase 6 완료 커밋

**태그: `phase6-benchmark`**

Phase 0~5가 "만들고 고친" 기록이라면 Phase 6은 **"만든 것이 실제로 그러한지 확인한"** 기록이다.
확인 결과 두 가지가 뒤집혔다 — 캐시는 이득이 없어 지웠고, 409 해석은 반증됐다.
